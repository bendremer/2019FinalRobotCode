...
public class AutoRun extends Command {
    ...
    protected void execute() {
        ...
        System.out.printf("Auto: dis to target %.2f\n", toTargetDis);
        if (toTargetDis < Constants.SHOOT_DIS && !isShoot) {
        ...
        DriveSub.DriveBase.arcadeDrive(calculateForwardPower(), calculateAngularPower(), false);
    }
    ...
    double calculateAngularPower() {
        ...
        double angularPower = P * angularError, linSpeed=Robot.driveSub.rateAvg; //TODO implement this
        // if the robot is moving, we need greater power to get it going
        double min_turn = Constants.MIN_TURN_SPEED * (Math.abs(linSpeed) < 2 ? 1.5 : 1);
