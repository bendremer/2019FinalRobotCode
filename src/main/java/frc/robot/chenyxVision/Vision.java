...
public class Vision{
    ...
    private Vision() {
        ...
        Notifier notifier=new Notifier(()->{
           isConnected=Timer.getFPGATimestamp()-timestamp<Constants.VISION_DISCONNECT_TIME;
        });
    ...
    private void updateTarget(){
        ...
        for(int i=0;i<targetCnt;i++){
            ...
            if((curLockedTarget!=null && lockIndex==-1 && output[i*4+2]<30 
            && targets[i].distance(curLockedTarget)<Constants.LOCK_ERROR))
                lockIndex=i;
        }
        ...
        // this calculates how much off the robot's heading is to the optimal heading, 
        //    which should be vertical to the plane of the target
        if(lockIndex!=-1){
            System.out.printf("vision: target lock updated\n");
    ...
    private Pt getCoordinate(double x, double lh, double rh, Pose pose){
        ...
        return new Pt(pose.x+Math.cos(angle)*dis, pose.y+Math.sin(angle)*dis).plus(Constants.CAMERA_OFFSET);
    }
